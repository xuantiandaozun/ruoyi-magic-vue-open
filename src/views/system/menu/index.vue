  ...
  if (row.menuId) {
    getMenu(row.menuId).then(response => {
      if (response) {
        form.value = response;
        // 确保必要的字段都有默认值
        form.value.menuType = response.menuType || 'M';
        form.value.isFrame = response.isFrame || '1';
        form.value.isCache = response.isCache || '0';
        form.value.visible = response.visible || '0';
        form.value.status = response.status || '0';
      }
      open.value = true;
      title.value = "修改菜单";
    });
