...
async function handleUpdate(row) {
  reset();
  try {
    const [deptOptionsRes, deptDataRes] = await Promise.all([
      listDeptExcludeChild(row.deptId),
      getDept(row.deptId)
    ]);
    
    if (deptDataRes) {
      deptOptions.value = proxy.handleTree(deptOptionsRes, "deptId");
      form.value = {
        ...deptDataRes,
        parentId: deptDataRes.parentId || 0
      };
      open.value = true;
      title.value = "修改部门";
    }
  } catch (error) {
    proxy.$modal.msgError("获取部门数据失败");
  }
}
